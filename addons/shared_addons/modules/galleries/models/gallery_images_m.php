...
class Gallery_images_m extends MY_Model
{
	...
	public function get_video_by_gallery($id, $options = array())
	{
		// Find new images on files
		$this->set_new_image_files($id);
	...
	{
		$query = $this->db
			->select('gallery_images.*, files.youtube,files.news_id,files.name, files.filename, files.extension, files.description, files.name as title, galleries.folder_id, galleries.slug as gallery_slug')
