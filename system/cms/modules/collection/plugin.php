...
class Plugin_collection extends Plugin
{
	...
	function artikel_terkait(){
		...
		if (!$slug or !$article = $this->collection_m->get_by('slug', $slug))
		{
			...
			foreach($key as $datakey){
				//echo $datakey;
				$words.="OR keyword LIKE '%".$datakey."%' AND created_on <= '".now()."' AND `default_collection`.`id` <> '".$article->id."'";
				//$this->db->or_like('keyword',$datakey);
				//$this->db->where('CONCAT(berita.date," ",SUBSTRING(berita.tgl,-5),":","00") <= ',date('Y-m-d H:i:s'));
				
			}
			
			$qq="SELECT * FROM (default_collection) WHERE `default_collection`.`id` <> '".$article->id."' AND `status` = 'live' AND created_on < '".now()."' AND keyword LIKE '%".$article->keyword."%'  ".$words." ORDER BY default_collection.id DESC LIMIT 5";
			...
			<tr>
			<td>
                         <a href="'.base_url().'collection/'.date('Y/m', $val->created_on) .'/'.$val->slug.'"><img   src="'.strip_image($val->body).'" alt="'.$val->title.'" width="35px"></a>
			 </td>
			 <td>
                        <a href="'.base_url().'collection/'.date('Y/m', $val->created_on) .'/'.$val->slug.'"><b>'.$val->title.'</b></a>
			 </td>
			 </tr>
			 </table>
	...
	function galeri_terkait(){
		$this->load->model('collection/collection_m');
		$category_id = $this->collection_m->get_where('collection_categories',array('id'=>$this->attribute('group')))->row();
		 
		if(!empty($category_id)){
		$folder_slug=$category_id->folder_id;
		
		$folder=$this->collection_m->get_where('file_folders',array('slug'=>$folder_slug,'parent_id' => 1))->row();
		if(!empty($folder)){
		$folder_id=$folder->id;
		$files=$this->collection_m->select('files.id as id,galleries.slug,gallery_images.id as fid')->join('galleries','galleries.folder_id = files.folder_id')->join('gallery_images','gallery_images.file_id = files.id')->get_where('files',array('files.folder_id'=>$folder_id,'files.youtube'=>'0'))->result();
	...
	function galeri_artikel(){ 
		...
		$files=$this->collection_m->select('files.id as id ,galleries.slug,gallery_images.id as fid')->join('galleries','galleries.folder_id = files.folder_id')->join('gallery_images','gallery_images.file_id = files.id')->get_where('files',array('files.collection_id'=>$this->attribute('collection_id'),'files.youtube'=>'0'))->result();
		$galeri='';
		if(!empty($files)){
			$vid='Foto Untuk Artikel Ini';
			if($this->uri->segment('1') == 'collection' && $this->uri->segment('2') == 'category'){
				$vid='Foto Terbaru';
			} 
			$galeri.='<div class="widget">
	...
	function twitterland_profile(){
	 ...
	 function twitterland_profile_vertical(){
		...
		if($this->uri->segment('2') == 'category' AND $this->uri->segment('1') <> 'collection'){
			$nama=$_SESSION['namablog'];
		}
		if($this->uri->segment('2') == 'page' AND $this->uri->segment('1') <> 'collection'){
			$nama=$_SESSION['namablog'];
		}
		if($this->uri->segment('2') == 'penulis' AND $this->uri->segment('1') == 'collection'){
			$nama=$_SESSION['namablog'];
		}
	 ...
	 function twitterland_profile_vertical_blog(){
		...
		if($this->uri->segment('1') == 'collection' and $this->uri->segment('2') <> 'penulis'){
			$nama=$_SESSION['namablog'];
	 ...
	 function video_artikel(){ 
		...
		$files=$this->collection_m->select('files.id as id ,files.youtube,galleries.slug,gallery_images.id as fid')->join('galleries','galleries.folder_id = files.folder_id')->join('gallery_images','gallery_images.file_id = files.id')->get_where('files',array('files.collection_id'=>$this->attribute('collection_id'),'files.youtube <>'=>'0'))->result();
		$galeri='';
		$vid='';
		if($this->uri->segment('1') == 'collection' && $this->uri->segment('2') == 'category'){
			$vid='';
	...
	function getID(){
		...
		if($this->uri->segment('1') == 'collection' AND $this->uri->segment('2') == 'category'){
			...
			$slug=$this->uri->segment('4');
			$article = $this->collection_m->get_by('slug', $slug);
	...
	function menu_level(){
	 ...
	 function menu_level3($id="",$level=""){
		...
		$idparent=$this->db->where('id',intval($id))->get('collection_categories')->row();
		if(!empty($idparent)){
		...
		if(!empty($idparent)){
		 ...
		 $DM=$this->db->where('id',intval($idparent))->get('collection_categories')->row();
		...
		if(!empty($datamenu)){
			...
			foreach($datamenu as $dat => $val){
				...
				if(!empty($val['uri'])){
					...
					$url=base_url().'collection/category/'.$val['slug'];
				}
	 ...
	 function menu_level4_ASLI($id="",$level=""){
		$lvl=menu_level($id);
		if($lvl == 5){
			$idparent=$this->db->where('id',$id)->get('collection_categories')->row();
			if(!empty($idparent)){
			$ids=$id; 
	...
	function menu_level_samping($id="",$lvl=""){  
		...
		if(!empty($datamenu)){
			...
			$menu.='</span>';
				 //$menu.='<b><a href="'.base_url().'collection/category/'.$url.'">'.$val['slug'].'</a></b> &nbsp;';
	 ...
	 function submenu($id="",$i="",$ids=""){
		...
		$categories = $this->db->where('navigation_group_id',$id)->where('show','1')->order_by('position','ASC')->get('collection_categories')->result();
		if(!empty($categories)){
