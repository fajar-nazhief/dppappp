...
class Rss extends Public_Controller
{
	...
	function index()
	{
		$posts = $this->pyrocache->model('collection_m', 'get_many_by', array(array(
			'status' => 'live',
			'limit' => $this->settings->item('rss_feed_items'))
		), $this->settings->item('rss_cache'));
		
		$this->_build_feed( $posts );		
		$this->data->rss->feed_name .= $this->lang->line('collection_rss_name_suffix');		
	...
	{ 
		...
		{
			redirect('collection/rss/index');
		}
		
		$posts = $this->cache->model('collection_m', 'get_many_by', array(array(
			'status' => 'live',
			'category' => $slug,
			'limit' => $this->settings->item('rss_feed_items') )
		), $this->settings->item('rss_cache'));
		...
		$this->_build_feed( $posts );		
		$this->data->rss->feed_name .= ' '. $category->title . $this->lang->line('collection_rss_category_suffix');		
		$this->output->set_header('Content-Type: application/rss+xml');
		$this->load->view('rss', $this->data);
	}
	...
	function _build_feed( $posts = array() )
	{
		...
		$this->data->rss->page_description = sprintf($this->lang->line('collection_rss_articles_title'), $this->settings->item('site_name'));
		...
		{        
			foreach($posts as $row)
			{
				...
				$row->link = site_url('collection/' .date('Y/m', $row->created_on) .'/'. $row->slug);
