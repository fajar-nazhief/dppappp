(function($) {
	$(function(){
		...
		$('#collection-options-tab ol li:first a').colorbox({
			srollable: false,
			innerWidth: 330,
			innerHeight: 280,
			href: BASE_URL + '/admin/collection/categories/create_ajax',
			onComplete: function() {
				$.colorbox.resize();
				...
				$('form#categories').submit(function(e) {
					...
					$.ajax({
						url: BASE_URL + '/admin/collection/categories/create_ajax',
						type: "POST",
					        data: form_data,
						success: function(data) {
							...
							if(obj.status == 'ok') {
								...
								//uniform workaround
								$('#collection-options-tab li:first span').html(obj.title);
