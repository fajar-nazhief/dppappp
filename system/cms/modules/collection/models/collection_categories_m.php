 ...
 */
class collection_categories_m extends MY_Model
{
	/**
	...
	protected $_table = 'default_collection_categories'; 
	 
	function collection_categories_m()
	{
		//parent::MY_Model();
		//$this->db=$this->db->dbprefix(''); 
    ...
    {
	$img='';
	if(!empty($img)){$img=$img;}else{
		$parent=$this->db->where('id',$input['navigation_group_id'])->get('collection_categories')->row();
	...
	if($this->uri->segment(4)=='search'){
		$where_condition=$this->uri->segment(5); 
		    $query =$this->db->query("
					SELECT default_collection_categories.* FROM `default_collection_categories` where title like '%".str_replace('%20','%',mysql_real_escape_string($where_condition))."%'  
					UNION
					SELECT default_collection_categories.* FROM `default_collection_categories` where slug like '%".str_replace('%20','%',mysql_real_escape_string($where_condition))."%'
	  ...
	  if(!empty($params['order'])){
		...
		return $this->db->get('collection_categories')->result();
	}
	...
	if($this->uri->segment(4)=='search'){
	 ...
	 $query =$this->db->query("SELECT count(default_collection.id) as jml,default_collection_categories.id as idc FROM default_collection ,default_collection_categories
                    WHERE default_collection.category_id = default_collection_categories.id and default_collection_categories.title like '%".str_replace('%20','%',mysql_real_escape_string($where_condition))."%' group by default_collection_categories.id");
	  return $query->result();
    }
	...
	function count_data($params=array()){
		...
		$where_condition=$this->uri->segment(5); 
		    $query =$this->db->query("
					SELECT * FROM `default_collection_categories` where title like '%".str_replace('%20','%',mysql_real_escape_string($where_condition))."%'  
					UNION
					SELECT * FROM `default_collection_categories` where slug like '%".str_replace('%20','%',mysql_real_escape_string($where_condition))."%'
					 "); 
