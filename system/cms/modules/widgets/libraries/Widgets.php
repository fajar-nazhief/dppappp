...
//		$_POST = $options;
	...
	protected function load_view($view, $data = array())
	{
		...
		$widget_name=explode('/',$path);
		 
		//echo $THEME_PATH =APPPATH . 'themes/'.$view;
		$datat =& $this->load->_ci_cached_vars;
		
		if(!empty($datat['template_views'])){
		@$path_theme = $datat['template_views'].'widget/'.$widget_name['5'].'/';
		}
		if(is_dir($path_theme)){
			return $view == 'display'

			? $this->parser->parse_string($this->load->_ci_load(array(
				'_ci_path'		=> $path_theme . $view . EXT,
				'_ci_vars'		=> $data,
				'_ci_return'	=> TRUE
			)), array(), TRUE)
