...
class My_m extends MY_Model {
	...
	protected $_table = 'kliping';

	function get_all()
	{   
		$this->db->select($this->MNAME.'.*, '.$this->MNAME.'_category.category_name AS category_title');
		$this->db->join($this->MNAME.'_category', $this->MNAME.'.category_id = '.$this->MNAME.'_category.id', 'left');
	...
	function get_many_by($params = array())
	{
		...
		if (($params['category']))
		{
			if (is_numeric($params['category']))
				$this->db->where($this->MNAME.'_category.id', $params['category']);
			else
				$this->db->where($this->MNAME.'_category.slug', $params['category']);
		}
