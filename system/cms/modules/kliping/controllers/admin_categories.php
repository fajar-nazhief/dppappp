...
class Admin_Categories extends Admin_Controller
{
	...
	public function index()
	{
		...
		if($this->input->post('search')){ 
			$this->db->like('category_name',$this->input->post('search'));
		} 
		
		$this->db->from($this->MNAME.'_category');
		$total_rows = $this->db->count_all_results();
		$pagination = create_pagination('admin/'.$this->MNAME.'/categories/index', $total_rows,50,5);
			
		// Using this data, get the relevant results
		if($this->input->post('search')){
			$this->categories_m->like('category_name',$this->input->post('search'));
		} 
		$categories = $this->categories_m->order_by('category_name')->limit($pagination['limit'])->get_all();
	...
	public function edit($id = 0)
	{
		...
		{		
			...
			$this->db->update('kliping_category', $data)
