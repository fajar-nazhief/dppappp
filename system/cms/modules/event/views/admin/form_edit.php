...
<?php endif; ?>
			...
			<div class="form-group">
				<label for="status">Event Ended Date</label>
				 <input type="text" name="date_end" value="<?php echo !($post->date_end)?date('d/m/Y'):date('d/m/Y',strtotime($post->date_end))?>" class="form-control datepicker">
			</div>
	...
	<div class="form-group">
				...
				<?php echo form_textarea(array('id' => 'alamat', 'name' => 'alamat', 'value' => $post->alamat, 'rows' => 5, 'class' => 'wysiwyg-simple')); ?>
	...
	<div class="form-group">
				...
				<label >#Tags</label>
				<select id="ahastag_form" name="ahastag_form" multiple="multiple" class="form-control input-sm chzn-select" onchange="myFunction(this, event)">
				<?php
				 				 			
				   foreach ($m_hastag as $val_list)
				   {
					   
					   $string_tag = $post->ahastag;
					   $db_sel  = explode(",", $string_tag);
					   $str_selected='';
						foreach ($db_sel as $line) {
							if ($val_list==$line){$str_selected='selected';}
							
						};
					   
						echo '<option value="'.$val_list.'" ' .$str_selected. '>'.$val_list.'</option>';
				   }	
				?>
				</select>				
				<!--?php echo form_dropdown('ahastag_form', array(lang('blog_no_category_select_label')) + $m_hastag, @$post->ahastag,' class="form-control input-sm chzn-select" multiple="multiple" id="ahastag_form" onchange="myFunction(this, event)"' ) ?-->
				...
				<div class="panel panel-default">
							...
							<div class="panel-tab clearfix">
								<ul class="tab-bar left">
									<li class="active "><a href="#home2" data-toggle="tab" style="color:#000"><i class="fa fa-home"></i> Deskripsi </a></li>
