<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
/**
 * Categories model
 *
 * @package		PyroCMS
 * @subpackage	Categories Module
 * @category	Modules
 * @author		Phil Sturgeon - PyroCMS Dev Team
 */
class Event_categories_m extends MY_Model
{
	/**
	 * Insert a new category into the database
	 * @access public
	 * @param array $input The data to insert
	 * @return string
	 */
	
	function get_all()
	{
		 
		return $this->db->get('blog_categories')->result();
	}
    
    
	function get($id)
	{
		 $this->db->where('id',$id);
		return $this->db->get('blog_categories')->row();
	}
	
	 
	public function insert($input = array())
	{
		$this->load->helper('text');
		$this->db->insert('blog_categories', array(
			'title'=>$input['title'],
			'navigation_group_id' =>$input['navigation_group_id'],
			'bahasa'			=> $this->input->post('bahasa'),
			'photo'			=> $this->input->post('photo'),
			'intro'			=> $this->input->post('intro'),
			'banner'			=> $this->input->post('banner'),
			'fa_icon'			=> $this->input->post('fa_icon'),
			'module_name'			=> $this->nama_modul, 
			'slug'=>url_title(strtolower(convert_accented_characters($this->input->post('slug'))))
		)); 
		 
		
		return $input['title'];
	}
    
	/**
	 * Update an existing category
	 * @access public
	 * @param int $id The ID of the category
	 * @param array $input The data to update
	 * @return bool
	 */
	public function get_by($slug){
		return $this->db->where('slug', $slug)->get('blog_categories')->row();
	}
	public function update($id, $input)
	{
		$this->db->where('id', $id);
		return $this->db->update('blog_categories',  array(
			'title'	=> $input['title'],
			'navigation_group_id' =>$input['navigation_group_id'], 
			'bahasa'			=> $this->input->post('bahasa'),
			'photo'			=> $this->input->post('photo'),
			'intro'			=> $this->input->post('intro'),
			'fa_icon'			=> $this->input->post('fa_icon'),
			'banner'			=> $this->input->post('banner'),
		        'slug'	=> url_title(strtolower(convert_accented_characters($this->input->post('slug'))))
		));
 
	}

	/**
	 * Callback method for validating the title
	 * @access public
	 * @param string $title The title to validate
	 * @return mixed
	 */
	public function check_title($title = '')
	{
		$this->db->where('slug',url_title($title));
		$data = $this->db->get('blog_categories')->row();
		return $data->slug;
		//return parent::count_by('slug', url_title($title)) > 0;
	}
	
	/**
	 * Insert a new category into the database via ajax
	 * @access public
	 * @param array $input The data to insert
	 * @return int
	 */
	public function insert_ajax($input = array())
	{
		$this->load->helper('text');
		return parent::insert(array(
			'title'=>$input['title'],
			'navigation_group_id' =>$input['navigation_group_id'], 
			'bahasa'			=> $this->input->post('bahasa'),
			'photo'			=> $this->input->post('photo'),
			'intro'			=> $this->input->post('intro'),
			'banner'			=> $this->input->post('banner'),
			'fa_icon'			=> $this->input->post('fa_icon'),
			//is something wrong with convert_accented_characters?
			//'slug'=>url_title(strtolower(convert_accented_characters($input['title'])))
			'slug' => url_title(strtolower($input['title']))
		));
	}
	
	//TREEE
	public function get_folders($id = 0)
	{  
		if ($id)
		{
			$this->folder_tree(0,0,$id);
		}
		elseif (!isset($this->_folder))
		{
			$this->folder_tree();
		}
		
		return $this->_folders;
	}
	
	public function folder_tree($parent_id = 0, $depth = 0,$bahasa="", &$arr = array())
	{
		 
		$arr = $arr ? $arr : array();

		if ($parent_id === 0)
		{
			$arr	= array();
			$depth	= 0;
		} 
		if(($bahasa)){
			//$this->db->where('bahasa',$bahasa);
		}
		$folders = $this
			->where('module_name',$this->nama_modul) 
			->order_by('title','ASC')
			->get_many_by(array('navigation_group_id' => $parent_id));
 
		if ( ! $folders)
		{
			return $arr;
		}

		static $root = NULL;

		foreach ($folders as $folder)
		{
			
			if ($depth < 1)
			{
				$root = $folder->id;
			}

//			$folder->name_indent		= repeater('&raquo; ', $depth) . $folder->name;
			$folder->root_id			= $root;
			$folder->depth				= $depth;
			//$folder->count_files		= sizeof($this->db->get_where('files', array('folder_id' => $folder->id))->result());
			$arr[$folder->id]			= $folder;
			$old_size					= sizeof($arr);

			$this->folder_tree($folder->id, $depth+1, $bahasa,$arr);

			//$folder->count_subfolders	= sizeof($arr) - $old_size;
		}

		if ($parent_id === 0)
		{
			foreach ($arr as $id => &$folder)
			{
				$folder->virtual_path		= $this->_build_asc_segments($id, array(
					'segments'	=> $arr,
					'separator'	=> '/',
					'attribute'	=> 'slug'
				));
			}

			$this->_folders = $arr;
		}

		if ($parent_id > 0 && $depth < 1)
		{
			foreach ($arr as $id => &$folder)
			{
				$folder->virtual_path = $this->_folders[$id]->virtual_path;
			}
		}

		return $arr;
	}
	
	public function _build_asc_segments($id, $options = array()) 
	{
		if ( ! isset($options['segments']))
		{
			return;
		}

		$defaults = array(
			'attribute'	=> 'name',
			'separator'	=> ' &raquo; ',
			'limit'		=> 0
		);

		$options = array_merge($defaults, $options);

		extract($options);

		$arr = array();

		while (isset($segments[$id]))
		{
			array_unshift($arr, $segments[$id]->{$attribute});
			$id = $segments[$id]->navigation_group_id;
		}

		if (is_int($limit) && $limit > 0 && sizeof($arr) > $limit)
		{
			array_splice($arr, 1, -($limit-1), '&#8230;');
		}

		return implode($separator, $arr);
	}
	
	public function delete($id){
		$this->db->where('id', $id);
		$this->db->delete('blog_categories');
	}
}