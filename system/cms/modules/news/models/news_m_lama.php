...
class News_m extends MY_Model
{
	...
	if (isset($params['limit']) && is_array($params['limit'])) $this->db->limit($params['limit'][0], $params['limit'][1]);
    	...
    	$this->db->select('news.*, default_c.title AS category_title, default_c.slug AS category_slug');
       	$this->db->join('default_news_categories as default_c', 'news.category_id = default_c.id', 'left');
    ...
    function count_data($params=array()){
		...
		$where_condition=$this->uri->segment(4); 
		    ...
		    $query =$this->db->query("
					SELECT default_a.*,default_b.title as category_title FROM default_news as default_a,default_news_categories as default_b where default_a.intro like '%".$where_condition."%'  and default_a.category_id=default_b.id
					UNION
					SELECT default_a.*,default_b.title as category_title FROM default_news as default_a,default_news_categories as default_b where default_a.body like '%".$where_condition."%'  and default_a.category_id=default_b.id
					UNION
					SELECT default_a.*,default_b.title as category_title FROM default_news as default_a,default_news_categories as default_b where default_b.title like '%".$where_condition."%'  and default_a.category_id=default_b.id
					");
    ...
    {
    	$this->db->join('default_news_categories c', 'news.category_id = c.id', 'left');
