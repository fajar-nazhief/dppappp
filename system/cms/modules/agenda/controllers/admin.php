...
class Admin extends Admin_Controller {
	...
	function download(){
	 ...
	 $this->db->select('tbl_agenda.*,default_profiles.first_name'); 
	 ...
	 if(!empty($user)){
		 ...
		 $this->db->where("tbl_agenda.createdby",$user); 
	 }
	 
	  
		
		$this->data->res = $this->db->get('tbl_agenda')->result();
		$this->db->set_dbprefix('tbl_');
	...
	public function edit($id = 0)
	{
		...
		{
			$created_on = $this->input->post('tgl_agenda');
		}

		else
		{
			$created_on = $post->tgl_agenda;
		}
		...
		if ($this->form_validation->run())
		{
			...
			if ($result)
			{
				$this->session->set_flashdata(array('success' => sprintf($this->lang->line('blog_edit_success'), $this->input->post('acara'))));
...
//				}
	...
	{
		if ( ! $this->my_m->check_exists('acara', $title, $this->id))
		{
