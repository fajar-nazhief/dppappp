	...
	<table border="0" class="table-list" id="list">
		...
		<?php  foreach ($files as $file): ?>
			<tr>
				...
				<td><img src="<?php  echo base_url().'uploads/default/files/'.$file->filename; ?>" width="100px"></td>
				<td><?php  echo lang('files.type_'.$file->type); ?></td>
				<td><?php  echo base_url().'uploads/default/files/'.$file->filename; ?></td>
				<td class="align-center"><?php  echo format_date($file->date_added); ?></td>
				<td class="align-center buttons buttons-small">
					 
					<?php 
					if($file->pilihan_editor){
						echo anchor('admin/files/selesai/' . $file->id, 'Selesai');
					}else{
						echo anchor('admin/files/pilih/' . $file->id, 'Pilih');
					}
					 ?>
