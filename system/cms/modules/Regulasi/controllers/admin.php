...
class Admin extends Admin_Controller {
	...
	public function create()
	{
		...
		if ($this->form_validation->run())
		{
			...
			$resupload = upload_data('regulasi');
			if($resupload){
				$post_data = array(
					...
					'source_array'		=> json_encode($resupload),
					'status'			=> 'approved',
					'created_on'		=> now(),
				...
				$this->session->set_flashdata('error', 'FIle tidak berhasil diupload');
			}
	...
	public function delete_data(){
		...
		$res = $this->db->where('id',$id)->get('regulasi')->row();
		 
		$data = json_decode($res->source_array);
		...
		$this->db->where('id',$id);
		$this->db->update('regulasi',array('source_array'=>json_encode($ff)));
	
	redirect('admin/regulasi/edit/'.$id);
	}
			...
			if(!empty($resupload)){
				...
				foreach($data as $val){ 
					...
					if($nama == $val->name){
						...
						$resupload[$i]['name']=$val->name;
					} 
				} 
			}else{
				 $resupload= json_decode($post->source_array);
			}
