...
$rss=get_web_page('http://rss.viva.co.id/get/showbiz'); 
    $rrss=$rss['content'];
...
foreach($feed->channel->item as $ul => $res) {
	...
	$date2=strtotime(date('d-m-Y H:i:s',strtotime($date1 . "-1 days")));
        ...
        if(empty($RESULT[0])){
            $insert="INSERT INTO default_news VALUES (NULL,'".str_replace(array("'",'"'),'',$res->title)."','viva.co.id','$slug',459,'0','".$databerita['image']."','".substr($databerita['body'],0,200)."...','$url','<img src=\"".$databerita['image']."\">".$databerita['body']."','NULL',".$date.",'','','','','','','live','','','','','','','','','0','','','','','','".$kateg[0]."'),";
...
foreach($html_chld->find('div.article') as $ul) {
...
if(empty($BODY)){
   ...
   foreach($html_chld->find('p') as $ul) {
      ...
      $data['image']=$IMAGE;
      $data['title']=$JUDUL;
      $data['body']=strip_tags(preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', "", $BODY),'<p><br>');
