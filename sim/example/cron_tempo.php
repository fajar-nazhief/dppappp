...
include('db.php');
$rss=get_web_page('http://www.tempo.co/rss/metro'); 
    $rrss=$rss['content'];
...
foreach($feed->channel->item as $ul => $res) {
	...
	$date2=strtotime(date('d-m-Y H:i:s',strtotime($date1 . "-1 days")));
        ...
        if(empty($RESULT[0])){
            $insert="INSERT INTO default_news VALUES (NULL,'".str_replace(array("'",'"'),'',$res->title)."','tempo.co.id','$slug',458,'0','".$databerita['image']."','".substr($databerita['body'],0,200)."...','$url','<img src=\"".$databerita['image']."\">".$databerita['body']."','NULL',".$date.",'','','','','','','live','','','','','','','','','0','','','','','','".$kateg[0]."'),";
...
$r=get_web_page($url);
 ...
 $rr=$r['content'];
   ...
   $html_chld = str_get_html($rr);
       ...
       foreach($html_chld->find('div.crop') as $ul) {
...
foreach($html_chld->find('div.artikel') as $ul) {
