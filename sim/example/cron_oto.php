...
mysql_select_db ('merdeka', $conn); 
if (!$conn) {
    die('Could not connect: ' . mysql_error());
}


$rss=get_web_page('http://www.otomotifnet.com/otonet/index.php/feed/10/Modifikasi/'); 
    $rrss=$rss['content'];
...
foreach($feed->channel->item as $ul => $res) {
     ...
     //print_r($res);
        ...
        $objRS = mysql_query ("select slug from default_news where slug = '".$slug."' "); 
        $RESULT= mysql_fetch_row($objRS);
        
        if(empty($RESULT[0])){
           echo $insert="INSERT INTO default_news VALUES (NULL,'".str_replace(array("'",'"'),'',$res->title)."','merdeka.com','$slug',459,'0','".$databerita['image']."','".substr($databerita['body'],0,200)."...','$url','<img src=\"".$databerita['image']."\">".$databerita['body']."','NULL',".$date.",'','','','','','','live','','','','','','','','','0','','','','','','".$kateg[3]."'),";
...
function get_channel($url=""){
 ...
 $rr=$r['content'];
   ...
   $html_chld = str_get_html($rr);
       ...
       foreach($html_chld->find('img') as $ul) {
...
foreach($html_chld->find('div.article fl') as $ul) {
