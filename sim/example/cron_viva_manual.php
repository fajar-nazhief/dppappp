...
$news_category=array('politik'=>'462','teknologi'=>'482','olahraga'=>'461','bisnis' => '460','sport' => '461','metro' => '458','otomotif'=>'484');
...
foreach($feed->channel->item as $ul => $res) {
     ...
     //print_r($res);
        ...
        if(empty($RESULT[0])){
           $insert="INSERT INTO default_news VALUES (NULL,'".str_replace(array("'",'"'),'',$res->title)."','viva.co.id','$slug',".$news_category[$category].",'0','".$databerita['image']."','".substr($databerita['body'],0,200)."...','$url','<img src=\"".$databerita['image']."\">".$databerita['body']."','NULL',".$date.",'','','','','','','live','','','','','','','','','0','','','','','','".$key[0]."'),";
