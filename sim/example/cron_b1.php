...
$rss=get_web_page('http://www.beritasatu.com/rss/megapolitan.xml'); 
    $rrss=$rss['content'];
...
foreach($feed->channel->item as $ul => $res) {
	...
	$date2=strtotime(date('d-m-Y H:i:s',strtotime($date1 . "-1 days")));
        ...
        if(empty($RESULT[0])){
            $insert="INSERT INTO default_news VALUES (NULL,'".str_replace(array("'",'"'),'',$res->title)."','www.beritasatu.com','$slug',458,'0','".$databerita['image']."','".substr(strip_tags($databerita['body']),0,200)."...','$url','<img src=\"".$databerita['image']."\">".str_replace(array("'",'"'),'',$databerita['body'])."','NULL',".$date.",'','','','','','','live','','','','','','','','','0','','','','','','".$kateg[0]."'),";
...
$r=get_web_page($url);
 ...
 $rr=$r['content'];
   ...
   $html_chld = str_get_html($rr);
       ...
       foreach($html_chld->find('div.article-body') as $ul) {
...
foreach($html_chld->find('h1.mtb10') as $ul) {
...
foreach($html_chld->find('div.f14') as $ul) {
  ...
  $data['image']=strip_tags($IMAGE);
  $data['title']=$JUDUL;
 
 $BODY = preg_replace('{<a[^>]+\>(.+?)</a>}s', " ", $BODY);
 $BODY = preg_replace('{<div id="inside-160">(.+?)</div>}s', " ", $BODY);
 $BODY = preg_replace('{<h2 class="block-title">(.+?)</h2>}s', " ", $BODY);
 $BODY = preg_replace('{<div class="block-inner low">(.+?)</div>}s', " ", $BODY);
