...
$rss=get_web_page('http://aceh.tribunnews.com/rss'); 
    $rrss=$rss['content'];
	...
	foreach($feed->channel->item as $ul => $res) {
        ...
        if(empty($RESULT[0])){
           $insert="INSERT INTO default_news VALUES (NULL,'".str_replace(array("'",'"'),'',$res->title)."','aceh.tribunnews.com','$slug',458,'0','".$databerita['image']."','".substr2(mysql_real_escape_string($databerita['body']),200)."...','$url','<img src=\"".$databerita['image']."\">".substr2(mysql_real_escape_string($databerita['body']),1200)."...','NULL',".$date.",'','','','','','','live','','','','','','','','','0','','','','','','".$kateg[0]."'),";
...
function get_channel($url=""){
	...
	$IMAGE='';
    foreach($html_chld->find('div.pb20') as $ul) {
